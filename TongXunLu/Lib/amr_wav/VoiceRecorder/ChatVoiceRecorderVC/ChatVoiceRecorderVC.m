...
#pragma mark - 启动定时器
- (void)startTimer{
    timer = [NSTimer scheduledTimerWithTimeInterval:0.2f target:self selector:@selector(updateMeters) userInfo:nil repeats:YES];
}
...
//        NSLog(@"峰值:%f",[recorder averagePowerForChannel:0]);
        ...
        //倒计时
        if (curCount >= kDefaultMaxRecordTime - 10 && curCount < kDefaultMaxRecordTime) {
            //剩下10秒
            recorderView.countDownLabel.text = [NSString stringWithFormat:@"录音剩下:%d秒",(int)(kDefaultMaxRecordTime-curCount)];
