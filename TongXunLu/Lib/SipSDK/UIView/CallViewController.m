...
{
  ETMySipSDK *app = [ETMySipSDK shareSipSDK];
...
- (UIImage *)findImageWithRecord:(ABRecordRef)record
{
  ...
  if (record && ABPersonHasImageData(record))
  {
    ...
    if (data)
      image = [[UIImage alloc] initWithData: (__bridge NSData *)data /*cache:YES*/];
  }
  return image;
}
  ...
  if (pool)
  {
    ...
    if (url != NULL)
    {
      ...
      if (record)
      {
        ...
        call.compositeName = (__bridge NSString *)ABRecordCopyCompositeName(record);
        call.uid = ABRecordGetRecordID(record);
        
        multiValue = ABRecordCopyValue(record, kABPersonPhoneProperty);
        index = ABMultiValueGetFirstIndexOfValue (multiValue, (__bridge CFTypeRef)(call.number));
