...
-(void)showImage
{
    ...
    NSMutableArray *groupMessageList = [groupMessagedb selectImageGroupMessageWithGid:_messageFrame.message.belongGroupId WithContactUid:_messageFrame.hostMemberId];
    NSMutableArray *imageUrlList = [self imageUrlListWithAllGroupMessage:groupMessageList];
    if (imageUrlList.count > 0) {
        ...
        browser.currentPhotoIndex = [self indexOfGroupMessageUrl:fullImagePath inImageUrlList:imageUrlList]; // 弹出相册时显示的第一张图片是？
        browser.photos = photos; // 设置所有的图片
        [browser show];
    }
}

-(NSMutableArray *)imageUrlListWithAllGroupMessage:(NSMutableArray *)groupMessageList
{
    NSMutableArray *imageList = [[NSMutableArray alloc] init];
    if (groupMessageList != nil) {
        for (GroupMessage *message in groupMessageList) {
...
-(NSUInteger)indexOfGroupMessageUrl:(NSString *)messageUrl inImageUrlList:(NSMutableArray *)imageUrlList
{
    NSUInteger i = 0;
    ...
    if (message.code == MessageCodeGroupText) {
        ...
        if (type == MessageContentText) {
            ...
            if(message.type == MessageTypeMe){
                ...
                messageFailedImageFrame = CGRectMake(_messageFrame.contentF.origin.x+10, _messageFrame.contentF.origin.y + _messageFrame.contentF.size.height/2-9, 18, 18);
            }
            else{
